...
#define T 10000  // number of timesteps
...
///* FILE I/0 SETUP */
    ...
    FILE *fpOutPsi;
    FILE *fpOutPhi;
    fpInit = fopen("./Data files/Initial states/(11-04-15)HeadOnCollision(N1=N2=1E60)(xi1=1E-2)(alpha=0)Initial.txt","r");   // initial conditions
    fpOutPsi = fopen("./Data files/Collision tests/(11-04-15)HeadOnCollision(N1=N2=1E60)(xi1=1E-2)(alpha=0).txt","w+");    // data output file for psi
...
///* SET PARAMETERS FROM FILE */
    ...
    fprintf(fpOutPsi, ", n->");
    fprintM(fpOutPsi, N);
    fprintf(fpOutPsi, ", M->%i, T->%i}\n\n", M, T); // put the trailing '{' back for a single array in Mathematica notation
...
///* INSTABILITY DETECTION */
        ...
        ///* WRITE PSI DATA TO FILE IN MATHEMATICA ARRAY FORMAT */
            ...
            for(i3=0; i3<=M; i3++){
                ...
                for(i2=0; i2<=M; i2++){
                    ...
                    for(i1=0; i1<=M-1; i1++){
                        ...
                        fprintM(fpOutPsi, creal(psi[i1][i2][i3][n1]));
                        fprintf(fpOutPsi, "+");
                        fprintM(fpOutPsi, cimag(psi[i1][i2][i3][n1]));
                        fprintf(fpOutPsi, "*I,");
                    }
                    //fprintf(fpOutPsi, "%e+%e*I", creal(psi[M-1][i2][i3][n]), cimag(psi[i1][i2][i3][n]));
                    fprintM(fpOutPsi, creal(psi[M][i2][i3][n1]));
                    fprintf(fpOutPsi, "+");
                    fprintM(fpOutPsi, cimag(psi[M][i2][i3][n1]));
                    fprintf(fpOutPsi, "*I");
                    if(i2==M)
                        fprintf(fpOutPsi, "}");
                    else
